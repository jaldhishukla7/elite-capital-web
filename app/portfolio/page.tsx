...
import { useBatchQuotes } from '@/lib/hooks/useMarketData'
...
export default function PortfolioPage() {
  ...
  const { data: liveQuotes } = useBatchQuotes(
    holdings.map((h) => h.symbol),
    { refreshInterval: 5000 }
  )

  const displayHoldings = holdings.map((holding) => {
    const live = liveQuotes?.find((q: any) => q.symbol === holding.symbol)
    return {
      ...holding,
      currentPrice: live ? live.ltP : holding.currentPrice
    }
  })
  ...
  const totalInvestment = displayHoldings.reduce((acc, h) => acc + h.buyPrice * h.shares, 0)
  const totalCurrentValue = displayHoldings.reduce((acc, h) => acc + h.currentPrice * h.shares, 0)
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between gap-4 flex-wrap md:flex-nowrap">
                    <Link href={`/stocks/${holding.symbol}`} className="flex-1 min-w-[200px] cursor-pointer">
                      <h3 className="font-bold text-[#1A1A1A] dark:text-white text-lg hover:text-[#44C2A4] transition-colors">
                        {holding.symbol}
                      </h3>
                    ...
                    <Link href={`/stocks/${holding.symbol}`} className="text-right cursor-pointer">
